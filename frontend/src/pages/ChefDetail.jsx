                    ...
                    {chef.dishes.map((d, i) => {
                      ...
                      return (
                        <button key={i} type="button"
                          ...
                          style={{ display:'flex', alignItems:'center', gap:7, padding:'5px 10px 5px 5px', borderRadius:8, border:`1.5px solid ${form.dishes.includes(d.name) ? 'var(--green)' : 'var(--g200)'}`, background: form.dishes.includes(d.name) ? 'var(--green-p)' : '#fff', cursor:'pointer', transition:'.15s', fontFamily:'var(--fb)' }}>
                          {imgUrl ? (
                            <img src={imgUrl} alt={d.name} style={{ width:28, height:28, borderRadius:6, objectFit:'cover' }}
                              onError={e => { e.target.style.display='none'; }}/>
                          ) : (
                            <span style={{ fontSize:'1.1rem' }}>🍽️</span>
                          )}
                          <span style={{ fontSize:'.78rem', fontWeight:600, color: form.dishes.includes(d.name) ? 'var(--green-d)' : 'var(--g700)' }}>
                            {d.name} · ₹{d.price}
                          </span>
                        </button>
                      );
